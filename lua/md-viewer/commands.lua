...
function M.setup()
  vim.api.nvim_create_user_command("MdViewerOpen", function(args)
    controller.open(args.args ~= "" and args.args or nil)
  end, { nargs = "?", complete = function() return { "right", "left", "below", "above" } end })
  vim.api.nvim_create_user_command("MdViewerClose", function() controller.close() end, {})
  vim.api.nvim_create_user_command("MdViewerToggle", function(args)
    controller.toggle(args.args ~= "" and args.args or nil)
  end, { nargs = "?", complete = function() return { "right", "left", "below", "above" } end })
